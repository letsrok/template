...
class MenuWidgetCustomTwo extends yupe\widgets\YWidget
{
    /**
    ...
    public function getList($arr) {
    	$menu_html = "";
    	if ($arr["itemOptions"]["class"]=="listItem submenuItem") {
    		$menu_html .= $this->getList($arr);
    	} else {
    		foreach ($arr as $val) {
    			if($val['visible']){
    				$menu_html .= '<li><a href="'.$val['url'][0].'">'.$val['label'].'</a>';
    			}
    		}
    	}
